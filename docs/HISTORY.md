...
## split_prints(): multi-print photos split into per-print crops (2026-08-16)

Follow-up to the landscape-print gap in the 2026-08-16 orientation entry
above: since `align_multi()` only rotates the whole merged group, it can't
read individual print content and so can't fix a sideways print inside one.
Splitting into per-print crops was next steps item 3; scoped narrow after
investigation showed the general case (2D grids, mixed orientations, prints
overlapping in both directions) is closer to general instance segmentation
than a heuristic — see `docs/PIPELINE.md` § 7 for the method and the ~30%
precision of an earlier, abandoned geometric-only attempt at the adjacent
sideways-detection problem, which is why this one leans on real evidence
(detected window positions) rather than blob shape alone.

**Real payoff, not just a narrower crop:** the exact file that started the
sideways investigation (`20260427_023359428`, two landscape cards stacked)
now splits into two individually-oriented, correctly-upright crops. Each
print gets its own `orient()` call on its own content, so this fixes
orientation for real, not just geometry — confirmed by eye on the actual
output, not just a border-ratio number.

**Verified against real overlap, including partial occlusion.** The 4-card
fan (`20260427_023727013`, the example the user pointed at first) initially
bled a sliver of each neighbour into every card but the outermost ones —
traced to assuming uniform spacing between overlapping prints, which real
overlap never quite is. Fixed by anchoring each print on its own detected
window centre instead of a grid, plus a small inward margin on estimated
edges. All 4 cards came out clean after the fix, confirmed by eye and by
`orient()`'s border ratio landing in the healthy 2.0–2.3 range on every one
(previously one was as low as 1.06, `orient()`'s own signal that a boundary
was off).

**Also verified the decline path fires correctly.** Swept every non-single
file in `chekis/main/`: 20 of 49 files split, the rest correctly declined --
including the six-print scattered layout and the 12-card 2D grid the user
also pointed at, both of which are genuinely outside this feature's scope.

**Real bug found integrating into the pipeline:** the fan-direction guess
(portrait vs landscape) was hardcoded to whichever orientation matched the
detected fan axis, which silently broke on real landscape-stacked files
(`20260427_023359428` initially found 0 usable window clusters). Fixed by
trying both orientation hypotheses and accepting only if exactly one
validates against the evidence — both or neither validating means genuine
ambiguity, same principle as the abandoned sideways heuristic.

**Pipeline surface area:** a split source now produces `stem_1.ext`,
`stem_2.ext`, ... instead of one `stem.ext`. `report.csv` gets one row per
output file, not per source; the skip-already-processed check now looks for
either shape; a reprocess that flips a source between single-output and
split-output cleans up whichever old files don't match its current shape,
so nothing is orphaned or duplicated across `balanced/`/`review/`.

**Elevated review rate on split crops** is a known, accepted characteristic,
not a bug — see `docs/PIPELINE.md` § 7's closing note. On this session's
`chekis/main/` batch: 36 in `balanced/`, 42 in `review/` (up from 40/26
before this feature — mostly newly-split files whose estimated margin came
up a little short, correctly caught by the existing finished-crop desk
check rather than shipped silently).
